...
router.post("/:id/stars", isUser, async (req, res) => {
  try {
    const product = await getOne(req.params.id);
    if (product.stars.some((a) => a == req.user._id)) {
      const userIndex = product.stars.indexOf((a) => a == req.user._id);
      product.stars.splice(userIndex, 1);
    } else {
      product.stars.push(req.user._id);
    }
    await product.save();
    res.json(product);
  } catch (err) {
...
router.put("/:id/saves", isUser, async (req, res) => {
  try {
    ...
    if (recipe.owner._id !== req.user._id) {
      const userSaveIndex = user.savedRecipes.indexOf(
